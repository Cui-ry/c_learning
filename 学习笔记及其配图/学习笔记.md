...
![alt text](image-8.png)双精度会好一点但也没好到哪里去？（这是因为十进制的数实际是转换成2进制计算的，但很多小数转换为2进制是会是循环小数，只能取它的近似值来计算）
10%3=1  余数为1，符号和%前的数字一致
10%-3=1 余数为1，符号和%前的数字一致
-10%3=-1 余数为-1，符号和%前的数字一致![alt text](image-9.png)

#include <math.h>包括高阶的数学运算函数，如pow()，sqrt()，sin()等
出现报错：![alt text](image-10.png)
